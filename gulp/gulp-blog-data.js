...
gulp.task('data', ['merge-blog-data']);

gulp.task('merge-blog-data', ['inject-blog-data-descriptions'], function () {
  var metadataPromises = [];
  var metadataArray = [];
  var collectionMetadata;

  glob.sync(config.injectedBlogMetadataSrc)
  ...
  function readCollectionMetadata(metadataPromises) {
    ...
    fs.readFile(config.blogCollectionMetadataSrc, function (err, data) {
      if (err) {
        deferred.reject(err);
      } else {
  ...
  function writeMergedMetadata(metadataArray) {
    ...
    fs.writeFile(config.combinedBlogDataFilePath, combinedMetadataString, function (err) {
      if (err) {
        deferred.reject(err);
      } else {
        deferred.resolve();
...
gulp.task('inject-blog-data-descriptions', function () {
  var streams = [];

  glob.sync(config.blogMetadataSrc)
  ...
  function injectDescription(streams, metadataFilePath) {
    ...
    var dataItemName = dataItemDirectoryPath.substring(config.blogDataPath.length + 1, dataItemDirectoryPath.length - 1);
    ...
    var stream = gulp.src(metadataFilePath)
        ...
        .pipe(gulp.dest(config.injectedBlogMetadataDist));
